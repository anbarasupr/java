...
public class GroupingByExamples {
	...
	public static void main(String[] args) throws Exception {
		...
		Map<Object, List<Integer>> map4 = persons.stream().collect(
				groupingBy(person -> Pair.of(person.salary(), person.department()), mapping(Person::id, toList())));
		// System.out.println(map4);

		// counting

		Map<Department, Long> map2 = persons.stream().collect(groupingBy(Person::department, counting()));
		// System.out.println(map2);

		Map<Double, Long> map3 = persons.stream().collect(groupingBy(Person::salary, counting()));
		...
		Map<Department, Long> map7 = persons.stream().filter(p -> p.salary() > 300d)
